...
export function BentoCard({ card, className }: BentoCardProps) {
  ...
  // Helper render function for list items
  ...
  return (
    <article
      ...
      className={cn(
        "group h-full", // Perspective wrapper for 3D flip
        "perspective-[2000px]",
    ...
    >
      <div className="relative w-full h-full duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
        ...
        >
          ...
          {card.id === "awards" && (
            <svg aria-hidden="true" className="absolute bottom-2 right-2 size-28 text-accent/10 pointer-events-none select-none transition-colors duration-500 group-hover:text-accent/25" viewBox="0 0 100 100">
        ...
        {/* ================= BACK FACE ================= */}
        {card.backTitle && card.backItems && (
          <div
            className={cn(
              "absolute inset-0 w-full h-full flex flex-col bg-background/20 backdrop-blur-md border border-accent/40 shadow-xl shadow-accent/10 p-6 sm:p-8 lg:p-10 rounded-sm overflow-hidden",
              "[backface-visibility:hidden] [transform:rotateY(180deg)]"
            )}
          >
            {/* Minimal Background pattern for back face */}
            <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_100%)]" />
            
            <div className="flex flex-1 flex-col gap-5 sm:gap-6 relative z-10">
              <div className="flex flex-col gap-3">
                <SectionLabel className="text-accent/80 hover:text-accent">{card.label} // EXPANDED</SectionLabel>
                <h3 className={cn("text-balance font-medium tracking-tight text-accent", isFeature ? "text-2xl sm:text-3xl lg:text-4xl" : "text-xl sm:text-2xl")}>
                  {card.backTitle}
                </h3>
